...
//const TEMPLATE_URL = "/templates/Template_Unggah_Data_Siswa_SMK.xlsx";
...
//    saveAs(TEMPLATE_URL, "Template_Data_Siswa_SMK.xlsx");
//  };
...
//    // === Validasi: hanya .xlsx ===
//    const isXlsxName = /\.xlsx$/i.test(f.name);
//    const isXlsxMime =
//      f.type === "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";

//    if (!(isXlsxName || isXlsxMime)) {
//      alert("Format file tidak valid. Harap pilih file Excel (.xlsx).");
//      e.target.value = "";
//      return;
//    }
...
//                />
//              </svg>
//              <span className="underline">Template_Data_Siswa_SMK.xlsx</span>
//            </button>
//          </div>
...
//              Hanya mendukung file dengan format <strong>.xlsx</strong>
//            </p>
...
import { MdHighlightOff } from "react-icons/md";
import SuccessModal from "@/app/components/registrasi/PopupBerhasil";
import { uploadSiswaBulk } from "@/lib/api-unggah-data-siswa";
...
type Props = {
  sekolahId?: number; // opsional; kalau tidak di-pass, akan ambil dari localStorage
};

const UploadExcel: React.FC<Props> = ({ sekolahId }) => {
  const [file, setFile] = useState<File | null>(null);
  ...
  useEffect(() => {
    if (sid != null) return;
    const v = Number(window.localStorage.getItem("sekolah_id"));
  ...
  const handleUpload = async () => {
    ...
    setLoading(true);
    try {
      await uploadSiswaBulk(
        file,
        sid /* , 'file' kalau field backend bukan 'file' */
      );
      setShowSuccess(true);
      setFile(null);
      if (inputRef.current) inputRef.current.value = "";
        ...
        <div className=" w-[332px] tablet:w-[526px] desktop:w-[629px]">
          ...
          <div className="flex justify-end mt-4">
            <button
              ...
              onClick={handleUpload}
              disabled={loading}
              className="px-4 py-2 rounded-[5px] bg-[#0F67B1] text-white disabled:opacity-50"
            >
              {loading ? "Mengunggah..." : "Unggah"}
            </button>
          </div>
