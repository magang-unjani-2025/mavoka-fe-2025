...
type Props = {
  ...
  primaryText?: string;
  showCloseIcon?: boolean;
  duration?: number; // auto close (ms); default 2500
  /** set false kalau mau render inline (jarang perlu) */
  usePortal?: boolean;
};
...
export default function SuccessModal({
  ...
  // --- Hindari SSR mismatch untuk portal ---
        ...
        >
          ...
          >
            {showCloseIcon && (
              <button
                onClick={onClose}
                className="absolute right-3 top-3 rounded-none text-2xl leading-none text-gray-500 hover:text-gray-700"
                aria-label="Tutup"
              >
                &times;
              </button>
  ...
  if (!usePortal) return content;          // render inline (opsional)
  if (!mounted) return null;               // tunggu mount sebelum portal
