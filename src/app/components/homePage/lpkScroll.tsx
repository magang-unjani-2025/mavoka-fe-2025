...
export default function TrainingCarousel() {
  ...
  const getVisibleLogos = () => {
    ...
    for (let i = -range; i <= range; i++) {
      const index = (currentIndex + i + trainingCenters.length) % trainingCenters.length;
      visible.push({
        ...trainingCenters[index],
        isCenter: i === 0,
      });
  ...
  const handleTouchEnd = () => {
    ...
    if (distance > 50) setCurrentIndex((prev) => (prev + 1) % trainingCenters.length);
    else if (distance < -50) setCurrentIndex((prev) => (prev - 1 + trainingCenters.length) % trainingCenters.length);
      ...
      <div className="max-w-[1154px] mx-auto px-6 text-center">
        ...
        <div className="flex justify-center my-3">
          <svg width="59" height="9" viewBox="0 0 59 9" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect x="0.44043" width="58" height="9" rx="4.5" fill="url(#paint0_linear_1456_3879)" />
            <defs>
              <linearGradient id="paint0_linear_1456_3879" x1="-21.5568" y1="-5.6056" x2="-21.1726" y2="7.15909" gradientUnits="userSpaceOnUse">
                <stop stopColor="#B2EBF2" />
                <stop offset="1" stopColor="#D1C4E9" />
              </linearGradient>
            </defs>
        ...
        >
          {getVisibleLogos().map((center, idx) => (
            <div
              key={idx}
              className={`flex flex-col items-center transition-transform duration-500 ${center.isCenter ? "scale-125" : "scale-100"}`}
            >
              <div className="rounded-full border-2 border-blue-400 flex items-center justify-center bg-white" style={{ width: "90px", height: "90px" }}>
                {center.logo ? (
                  <Image src={center.logo} alt={center.name} width={60} height={60} />
                ) : (
                  <HiUser className="w-12 h-12 text-black" />
                )}
              </div>
              {center.isCenter && <p className="mt-2 text-sm font-medium">{center.name}</p>}
            </div>
          ))}
        </div>
      </div>
