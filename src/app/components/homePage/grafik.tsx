    ...
    <section className="mt-10">
      <Container>
        <div className="relative overflow-hidden rounded-[15px] bg-[#0F67B1] p-6 tablet:p-8 text-white">
          ...
          <svg
            className="absolute right-0 top-0 z-0 hidden desktop:block"
            width="194"
            height="126"
            viewBox="0 0 194 126"
            fill="none"
          ...
          <svg
            className="absolute right-0 top-0 z-10 hidden desktop:block"
            width="162"
            height="113"
            viewBox="0 0 162 113"
            fill="none"
          ...
          <h2 className="relative z-10 mb-2 text-xl font-semibold tablet:text-2xl">
            ...
            <br />
            Lembaga Pelatihan dan Sekolah{" "}
            <span className="text-yellow-400">terbaik</span> di seluruh
            Indonesia
          </h2>
          <p className="relative z-10 mb-6 text-sm">
            MAVOKA membantu para siswa SMK untuk belajar secara langsung di
            dunia industri
          </p>
          ...
          <div className="relative z-10 grid grid-cols-1 tablet:grid-cols-2 desktop:grid-cols-4 gap-4 tablet:gap-6">
            {stats.map((stat, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 py-3 tablet:py-4 px-1 tablet:px-3"
              >
                <span
                  aria-hidden
                  className="block h-10 tablet:h-12 desktop:h-14 w-px bg-white/80"
                />
                <span className="ml-3 shrink-0">{stat.icon}</span>
                <div className="ml-1">
                  <p className="text-4xl tablet:text-5xl font-bold leading-none">
                    {stat.value}
                  </p>
